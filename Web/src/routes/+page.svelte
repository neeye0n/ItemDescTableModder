<script lang="ts">
  ...
  function handleDrop(event: DragEvent) {
    ...
    isDragOver = false;
    if (file) {
      showToast("⚠️ I only make this one at a time.", "warning");
      return;
    }
    ...
    if (event.dataTransfer?.files && event.dataTransfer.files.length > 0) {
      ...
      if (droppedFile.name.endsWith(".lua")) {
        ...
        showToast("❌ Heyyy thats not a .lua file!", "error");
      }
    }
  }
  ...
  function generate(event: Event) {
    event.preventDefault();
    if (!file) {
      showToast("⚠️ Give the a file first", "warning");
      return;
    }
  ...
  >
    <div class="card-body space-y-4">
      ...
      <fieldset class="fieldset">
        ...
        {:else}
          <p class="text-info text-center text-xs">
            Where is your itemInfo_EN.lua file? 👀
          </p>
        {/if}
      </fieldset>
      ...
      <div class="card-actions justify-end">
        <button
          ...
          on:click={generate}
        >
          {file ? "Lets do this!" : "Give me the file first!"}
        </button>
      </div>
    </div>
  </div>
